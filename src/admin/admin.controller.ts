...
export class AdminController {
  ...
  @UseFilters(new FileSizeExceptionFilter())
  @UseInterceptors(FileInterceptor('file', multerConfig))
  async uploadTimeslot(@UploadedFile() file: Express.Multer.File) {
    if (!file) {
      throw new BadRequestException('No file uploaded.');
    }
    await this.adminService.uploadTimeslot(file);
    return { message: 'Timeslot Upload successfully' };
  }
  ...
  async generateTimetable() {
    await this.adminService.generateTimetable();
    return { message: 'Timetable generated successfully' };
  }

  @Get('timetable')
  async getAllTimetable() {
    return await this.adminService.getAllTimetable();
  }
  @Get('time-slot')
  async getAllTimeslot() {
    return await this.adminService.getAllTimeslot();
  }

  @Post('delete-timetable')
  async deleteSchedule() {
    await this.adminService.deleteSchedule();
    return { message: 'All timetable deleted successfully' };
  }
}
