...
export class AuthService {
  ...
  async generateUserToken(email: string, id: string, role: string) {
    ...
    return {
      access_token: this.jwtService.sign(payload, { expiresIn: '3d' }),
    };
  }

  async storeUserToken(token: string, id: string): Promise<void> {
  ...
  async refreshToken(refreshToken: string): Promise<{ access_token: string }> {
    try {
      const token = await this.refreshTokenService.findOne(refreshToken);
      if (!token) {
